...
const Admin = () => {
  ...
  const [cuadriculaActualizar, setCuadriculaActualizar] = useState('');
  const [nuevoNombre, setNuevoNombre] = useState('');
  const [isLoadingActualizar, setIsLoadingActualizar] = useState(false);
  ...
  const handleActualizar = async (e) => {
    ...
    if (!cuadriculaActualizar.trim() || !nuevoNombre.trim()) {
      setError('Por favor completa todos los campos');
      return;
    }

    const cuadricula = parseInt(cuadriculaActualizar);
    if (isNaN(cuadricula) || cuadricula <= 0) {
      setError('El número de cuadrícula debe ser un número positivo');
      return;
    }

    setIsLoadingActualizar(true);
    setError('');
    setMensaje('');

    try {
      await compradorService.actualizarComprador(cuadricula, { nombre: nuevoNombre });
      setMensaje(`Nombre del comprador de cuadrícula ${cuadricula} actualizado exitosamente`);
      setCuadriculaActualizar('');
      setNuevoNombre('');
      cargarCompradores(); // Recargar la lista
    } catch (error) {
      if (error.response && error.response.status === 404) {
        ...
        setError('Error al actualizar el comprador. Por favor intenta de nuevo.');
      }
      console.error('Error:', error);
    } finally {
      setIsLoadingActualizar(false);
    }
  };
  ...
  return (
    <div className="admin-container">
      ...
      <div className="admin-content">
        ...
        <div className="form-card">
          <h2>Actualizar Nombre de Comprador</h2>
          <form onSubmit={handleActualizar}>
            <div className="form-group">
              <label htmlFor="cuadriculaActualizar">Número de Cuadrícula:</label>
              <input
                type="number"
                id="cuadriculaActualizar"
                value={cuadriculaActualizar}
                onChange={(e) => setCuadriculaActualizar(e.target.value)}
                placeholder="Ingresa el número de cuadrícula a actualizar"
                min="1"
                required
              />
            </div>
